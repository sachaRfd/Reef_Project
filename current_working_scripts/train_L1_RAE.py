...
import sys
from Robust_autoencoder_dropout import *  # noqa   # To reverse remove _dropout
    ...
    if tf.config.list_physical_devices('GPU'):
        ...
        with tf.device('/GPU:0'):
            ...
            with tf.compat.v1.Session() as sess:
                rae = RDAE(sess=sess, lambda_=lambda_, layers_sizes=[input_size*input_size, 400, 200]) #2_000, 1_500, 800])  # noqa
                L, S, list_of_costs, list_of_costs_testing = rae.fit(data, test_data, sess=sess, learning_rate=.0005, batch_size=200, inner_iteration=inner_iteration,  # noqa
                               iteration=iteration, verbose=True, save_directory=save_directory)
                
                # Setup convergence plots: 
                list_of_costs = np.array(list_of_costs)
                list_of_costs_testing = np.array(list_of_costs_testing)
                print(f"Shape of the list of the costs of training: {list_of_costs.shape}")
                print(f"Shape of the list of the costs of testing: {list_of_costs_testing.shape}")
                ...
                ax.legend()  # Show the labels
                plt.savefig((save_directory + "Convergence_plot.png"))
                plt.close(fig)
                ...
                for i in range(5):
                    ...
                    ax[1].imshow(recon_train[i].reshape(input_size, input_size))
                    ax[1].set_title("Reconstructed Image from training")
                    plt.savefig((save_directory + f"_training_example_{i}.png"))
                    plt.close(fig)

                # Plot some example L and S matrices from the training process: 
                ...
                # Get the recognition from the test data:
                ...
                for i in range(9):
                    ...
                    image.write(recon_test[i].reshape(input_size, input_size), 1)
                    image.close()

                
                # Save Model: Create a saver object
                saver = tf.compat.v1.train.Saver()
                saver.save(sess, save_directory + "model")  # 'Model_Checkpoints/training_1/model')
