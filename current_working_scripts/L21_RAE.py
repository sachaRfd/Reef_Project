...
class RobustL21Autoencoder(object):
    ...
    def fit(self, X, X_test, sess, save_directory, learning_rate=0.15, inner_iteration=50,  # noqa
        ...
        for it in range(iteration):
            ...
            recon_ = self.getRecon(X, sess)  # Get recon  # Before was X_test but we will keep it
            # Create new Directory to store the images in:
            ...
            if it % 20 == 0:
                ...
                for i in range(100):
                    row, col = divmod(i, 10)
                    ax[row][col].imshow(np.reshape(X[i, :], (28, 28)))  # Before was using Testing set, but let's keep Training set to visualise it
                    ax[row][col].axis('off')
