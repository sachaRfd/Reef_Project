...
class dataset():
    ...
    def load_dataset(self):
        ...
        for i, path in enumerate(self.files):
            ...
            # Get only the First band in the data:   --> RED Band
            # data = data[:, :, 0]

            # Get the second band in the data:  --> GREEN Band
            data = data[:, :, 1]

            # Get the third band in the data:  --> BLUE Band
            # data = data[:, :, 2]
            ...
            if data.shape[0] < self.max_shape[0] and data.shape[1] < self.max_shape[1]:  # noqa
                ...
                # Resize the image to correct Shape:
                # data = cv2.resize(data, (self.max_shape[0], self.max_shape[1]))  # noqa
