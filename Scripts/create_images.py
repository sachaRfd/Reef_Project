        ...
        for band in self.bands:
            if 'B02' in band:
                with rasterio.open(self.path_to_image + band) as src:
                    blue_array = src.read(1)
                    # Min Max Normalisation:
                    # blue_array = (blue_array - np.min(blue_array)) / (np.max(blue_array) - np.min(blue_array)) # noqa
                    trueColor.write(blue_array, 3)
                print("Blue Done")
            elif 'B03' in band:
                with rasterio.open(self.path_to_image + band) as src:
                    green_array = src.read(1)
                    # Min Max Normalisation:
                    # green_array = (green_array - np.min(green_array)) / (np.max(green_array) - np.min(green_array)) # noqa
                    trueColor.write(green_array, 2)
                print("Green Done")
            elif 'B04' in band:
                with rasterio.open(self.path_to_image + band) as src:
                    red_array = src.read(1)
                    # Min Max Normalisation:
                    # red_array = (red_array - np.min(red_array)) / (np.max(red_array) - np.min(red_array)) # noqa
                    trueColor.write(red_array, 1)
                print("Red Done")
        trueColor.close()
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: python script.py <Path/to/folder>  <Number of Bands>  <Output File name>")  # noqa
        sys.exit(1)
