...
import rasterio
from train_test_split import *  # noqa
    ...
    if tf.config.list_physical_devices('GPU'):
        ...
        with tf.device('/GPU:0'):
            ...
            with tf.compat.v1.Session() as sess:
                ...
                L, S, list_of_costs, list_of_costs_testing = rae.fit(data, test_data, sess=sess, learning_rate=.001, batch_size=200, inner_iteration=inner_iteration,  # noqa
                ...
                for i in range(iteration):
                    ...
                    ax.plot(range(0, inner_iteration),
                            list_of_costs[i], color="blue", label="Training Loss")  # noqa
                    ax.plot(range(0, inner_iteration),
                            list_of_costs_testing[i], color="orange", label="Testing Loss")  # noqa
                    ax.set_title(
                        f"Convergence plot for the {i}'th iteration of the Robust Autoencoder")  # noqa
                ...
                list_of_costs_all = np.ravel(
                    list_of_costs)  # Unravel the vector
                list_of_costs_testing_all = np.ravel(
                    list_of_costs_testing)  # Unravel the vector

                # Plot the whole Convergence plot:
                fig, ax = plt.subplots(1, figsize=(10, 5))
                ax.plot(range(0, iteration*inner_iteration),
                        list_of_costs_all, color="blue", label="Training Loss")
                ax.plot(range(0, iteration*inner_iteration),
                        list_of_costs_testing_all, color="orange", label="Testing Loss")  # noqa
                ax.set_title(
                    "Convergence plot for all iterations of the Robust Autoencoder")    # noqa
                ax.legend()  # Show the labels
                ...
                # Plot some example L and S matrices from the final training process  # noqa
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 5:
        print("Usage: python script.py <Path to Clipped Images to resize> <Size of Images> <Lambda Value> <Path to Save>")  # noqa
        sys.exit(1)
